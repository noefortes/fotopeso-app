...
export class PagarmeProvider implements IPaymentProvider {
  ...
  async initialize(config: PaymentProviderConfig): Promise<void> {
    // TODO: Initialize Pagar.me SDK with config.apiKey
    console.log(`[Pagar.me] Provider initialized for ${config.environment} environment`);
  }

  async createCustomer(data: CreateCustomerData): Promise<PaymentResult<PaymentCustomer>> {
    return { success: false, error: "Pagar.me provider not yet implemented" };
  }
