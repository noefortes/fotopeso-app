...
export class PagseguroProvider implements IPaymentProvider {
  ...
  async initialize(config: PaymentProviderConfig): Promise<void> {
    // TODO: Initialize PagSeguro SDK with config.apiKey
    console.log(`[PagSeguro] Provider initialized for ${config.environment} environment`);
  }

  async createCustomer(data: CreateCustomerData): Promise<PaymentResult<PaymentCustomer>> {
    return { success: false, error: "PagSeguro provider not yet implemented" };
  }
